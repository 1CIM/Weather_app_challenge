...
const Weeklyweather = ({ dailyWeather }) => {
  ...
  for (let i = 1; i < dailyWeather.length; i++) {
    ...
    forecastList.push(
      <Grid.Column key={i} textAlign="center" data-cy="seven-days-list-items">
        ...
        <Header.Content data-cy="daily-weather">
          {dailyWeather[i].weather[0].description}
        </Header.Content>
        <Header.Content data-cy="daily-temp">
          {dailyWeather[i].temp.day}°C
        </Header.Content>
      </Grid.Column>
    );
  }
  ...
  return (
    <Grid columns={7} celled id="hou">
      <Grid.Row id="opaci" color="black" inverted data-cy="seven-days-list">
        {forecastList}
      </Grid.Row>
    </Grid>
  );
